...
export function sendUpdateNote(data) {
  return (dispatch) => {
    ...
    return apiUtils
      .callAPI(`/api/notes/${data._id}`, "PUT", data)
...
export function fetchList(...listArgs) {
  return (dispatch) => {
    ...
    return apiUtils
      .callAPI(apiTarget)
      .then((json) => dispatch(receiveNoteList(json, listArgs)));
  };
}
